<template>
  <div class="orderConfirm-page">
     <ul class="cart-status">
       <li class="cart-item" v-for="(item, index) in cartStatus"  :class="{'active': index === itemIndex}" :key="index">
    ...
    <div class="cart-content">
      <ul class="cart-title">
        <li v-for="item in cartTitle" class="cart-des"  :key="item">{{item}}</li>
      </ul>
    </div>
      <ul class="cart-list">
        <li v-for="(item, index) in productList" class="cart-item" v-if="item.checked == 1" :key="index">
      ...
      <div class="total-btn">
        ...
        <button class="btn-confirmPay btn" @click="_payMent">
          <router-link to="{path:'/address'}">确认支付</router-link>
          </button>
      </div>
  </div>
</template>
